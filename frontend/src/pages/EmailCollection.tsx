  ...
  const loadDataFromStorage = () => {
    try {
      ...
      if (storedData) {
        ...
        if (data.collectionStatus) setCollectionStatus(data.collectionStatus);
        if (data.isCollecting) setIsCollecting(data.isCollecting);
        if (data.logs) setLogs(data.logs);
        if (data.collectionResults) setCollectionResults(data.collectionResults);
        if (data.totalResults) setTotalResults(data.totalResults);
        if (data.currentPage) setCurrentPage(data.currentPage);
        if (data.pageSize) setPageSize(data.pageSize);
        message.success('已恢复之前的采集数据');
      }
    } catch (error) {
