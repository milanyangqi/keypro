...
export const getCollectionResults = async (req: express.Request, res: express.Response) => {
  try {
    ...
    // 根据taskId查询对应的采集结果
    const numbers = await WhatsAppNumber.find({ uploader: userId, taskId })
      .skip(skip)
      .limit(parseInt(limit as string))
      .sort({ uploadTime: -1 });
    
    const total = await WhatsAppNumber.countDocuments({ uploader: userId, taskId });
...
const simulateCollectionProcess = async (task: ICollectionTask) => {
  ...
  const interval = setInterval(async () => {
    ...
    try {
      ...
      console.error('Error creating simulated WhatsApp numbers:', error);
    }
